...
<body data-background-color="bg3">
		...
		<div class="main-panel">
			<div class="content">
				...
				<div class="page-inner">
					...
					<?php  
						$query=mysqli_query($koneksi, "SELECT *	FROM tb_job j
						left join tb_karyawan k on k.nik=j.di_tujukan where status in ('Close','Open','Progress') and progress='25' and di_tujukan='$g' ;");
						while($row=mysqli_fetch_array($query)){
						$idj=$row['id_job']; 
					 ?>
						...
						<?php  
						$query=mysqli_query($koneksi, "SELECT *	FROM tb_job j
						left join tb_karyawan k on k.nik=j.di_tujukan where status in ('Close','Open','Progress') and progress='50' and di_tujukan='$g' ;");
						while($row=mysqli_fetch_array($query)){
						$idj=$row['id_job']; 
					 ?>
						<div class="col-12 col-sm-6 col-md-2">
						...
						<?php  
						$query=mysqli_query($koneksi, "SELECT *	FROM tb_job j
						left join tb_karyawan k on k.nik=j.di_tujukan where status in ('Close','Open','Progress') and progress='75'  and di_tujukan='$g';");
						while($row=mysqli_fetch_array($query)){
						$idj=$row['id_job']; 
					 ?>
						<div class="col-12 col-sm-6 col-md-2">
						...
						<?php  
						$query=mysqli_query($koneksi, "SELECT *	FROM tb_job j
						left join tb_karyawan k on k.nik=j.di_tujukan where status in ('Close') and progress='100' and di_tujukan='$g' ;");
						while($row=mysqli_fetch_array($query)){
						$idj=$row['id_job']; 
					 ?>
						<div class="col-12 col-sm-6 col-md-2">
	...
	<script>
		...
		var myPieChart = new Chart(pieChart, {
			...
			data: {
				datasets: [{
					data: [<?php $jumlah_teknik = mysqli_query($koneksi,"SELECT * FROM tb_job where status in ('Open','Pending') and di_tujukan='$g' and  update_at > DATE_ADD(CURDATE(), INTERVAL -7 DAY)");
							echo mysqli_num_rows($jumlah_teknik); ?>,
