							...
							<div class="card">
								...
								<div class="card-body">
									...
									<table id="multi-filter-select" class="display table table-striped table-hover" style="zoom:95%">
											...
											<?php
												$query=mysqli_query($koneksi, "SELECT *	FROM tb_job j
												left join tb_karyawan k on k.nik=j.di_tujukan where status='Done' ;");
												$no=0;
												$no++;
												while($row=mysqli_fetch_array($query)){
													$idj=$row['id_job'];
											...
											?> 
												<tr>
													...
													<td><?php echo $row['create_job'] ?></td>
													<td><?php echo $row['update_at'] ?></td>
													<td> <?php $create    =new DateTime($row['create_job']);
																							$today      =new DateTime($row['update_at']);
																							$diff = $today->diff($create);
																							echo $diff->d; echo " Hari";?></td>
													<td><b><?php echo $row['aproved_by'] ?></td></b>
													<td width="4%">
														<div class="form-button-action">
															<a href="forms3.php?id=<?=$idj;?>" data-toggle="tooltip" title="" class="btn btn-link btn-primary btn-lg" data-original-title="Edit">
											...
											<div class="modal fade" id="exampleModalLong<?=$idj;?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
												<div class="modal-dialog" role="document">
													...
													<form method="post" role="form" enctype="multipart/form-data" class="form-horizontal">
														...
														<div class="modal-footer">
															<button type="submit" name="deletejob2" class="btn btn-danger">Delete</button>
														</div>
													</fomr>
													</div>
												</div>
